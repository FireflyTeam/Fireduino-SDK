 /**
 * @file Firemata.cpp
 * @brief Definitions Firemata class for Fireduino
...
/**
 * Blink forever to indicate error to the onboard LEDs (if the board has an onboard LED).
 */
void FirmataClass::blinkError(void)
{
  for (;;) {
#if defined(VERSION_BLINK_PIN)
  if (blinkVersionDisabled) return;
  // flash the pin with the protocol version
  pinMode(VERSION_BLINK_PIN, OUTPUT);
  strobeBlinkPin(VERSION_BLINK_PIN, 20, 40, 100);
#endif
  }
}
...
/**
 * Send all strings
 * @param string An array of pointers to the char string,
 *               which ends with NULL.
 */
void FirmataClass::sendString(const char * const *string)
{
  const char *c;
  startSysex();
...
void FirmataClass::softSystemReset(void)
{
  int i;
...
void FirmataClass::strobeBlinkPin(byte pin, int count, int onInterval, int offInterval)
{
  byte i;
  for (i = 0; i < count; i++) {
#ifdef VERSION_BLINK_PIN_LOW_IS_ON
    delay(offInterval);
