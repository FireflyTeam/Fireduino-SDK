...
class PS2X {
  ...
  private:
    ...
    #else
      uint8_t maskToBitNum(uint8_t);
      unsigned int  _clk_mask;
      volatile unsigned int *_clk_oreg;

      unsigned int _cmd_mask;
      volatile unsigned int *_cmd_oreg;

      unsigned int _att_mask;
      volatile unsigned int *_att_oreg;

      unsigned int _dat_mask;
      volatile unsigned int *_dat_ireg;
    #endif
