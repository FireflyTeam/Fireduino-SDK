 /**
 * @file wifi_drv.c
 * @brief Definitions wifi_drv class for Fireduino
...
// 0.3 secs of delay to have the connection established
#define WL_DELAY_START_CONNECTION 300
...
private:
    ...
    static void scanCallback(const char *ssid, int ssid_len, int32_t rssi, uint8_t encType);
    static void doConnect(const char *ssid, const char *pass);	
	static void doConnect(unsigned int second,char *ssid);
	static void doConnect(char *ssid);
...
public:
	...
	static int8_t wifiSetNetwork(void);
	static int8_t wifiSetNetwork(unsigned int seconds);
